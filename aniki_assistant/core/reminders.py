...
FIX: периодические напоминания срабатывают с первого раза.
FIX: метод add_reminder() (chat_window вызывал .add()).
"""
...
class ReminderSystem:
    ...
    def _check_db_reminders(self):
        ...
        Это позволяет отсчитывать интервал с момента запуска, а не с эпохи.
        """
        now       = datetime.now()
        ...
        for r in reminders:
            ...
            if remind_at:
                # Напоминание в конкретное время
                try:
                ...
                if not last_triggered:
                    # FIX: первый запуск — помечаем время, НЕ стреляем
